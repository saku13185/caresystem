...
def get_db_connection() -> DatabaseConnector:
    return DatabaseConnector("care_system.db")
...
selected_res_code = st.sidebar.radio(
    label="노인 비식별 가상 코드 목록",
    options=[r["virtual_code"] for r in filtered_residents],
    label_visibility="collapsed"
)

# 선택된 노인의 상세 엔터티 매핑
selected_resident = next(r for r in residents if r["virtual_code"] == selected_res_code)
resident_id = selected_resident["id"]

# 4. 메인 대시보드 - 헤더 타이틀 및 등급 배지 렌더링
col_header, col_badge = st.columns([4, 1])
with col_header:
    st.markdown(f"<h1 style='margin:0; font-weight:800; color:#fff;'>Resident Dashboard: {selected_res_code}</h1>", unsafe_allow_html=True)
    st.markdown("<p style='color: #a0aec0;'>AttentionRNN 시계열 예측과 Double-step IQR 통계 기반의 독거노인 일상생활 예방 예보 관리 화면</p>", unsafe_allow_html=True)
...
with col_xai:
    ...
    st.markdown(f"""
        <div class='glass-card' style='background: rgba(255, 255, 255, 0.02); line-height: 1.6; white-space: pre-wrap; font-size: 0.95rem; border-color: rgba(255,255,255,0.05);'>
            {clean_report}
        </div>
    """, unsafe_allow_html=True)

with col_feedback:
    st.markdown("<h3 style='color: #fff;'>✏️ 사회복지사 예방 조치 피드백 관문</h3>", unsafe_allow_html=True)
    st.markdown("<p style='color: #a0aec0; font-size: 0.85rem;'>경보 정탐/오탐 판정 및 안부 확인 결과 영속 저장</p>", unsafe_allow_html=True)

    # 데이터베이스에서 기존 피드백/조치 상태 로드
    with db.get_connection() as conn:
        alert = conn.execute(
